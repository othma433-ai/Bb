# AL-thmany 4.0.0 — Stability & Smart Automation Release

## ما الجديد في 4.0.0

هذا الإصدار يركز على الاستقرار الفعلي للتطبيق المدمج Sender + Extractor بدل إضافة Delays أو ترقيعات مؤقتة:

- **قفل موحد لواجهة واتساب:** Sender / Extraction / Scan / Publish لا يستطيع اثنان منها التحكم بواجهة واتساب في الوقت نفسه.
- **حماية من الاتجاهين:** ليس فقط Extractor يمنع نفسه أثناء Sender؛ Sender أيضًا يرفض البدء أو استعادة التشغيل إذا كانت عملية أخرى تملك واجهة واتساب.
- **استعادة آمنة بعد إعادة إنشاء الخدمة:** Accessibility وShizuku يعيدان تأكيد ملكية Sender فقط إذا لم تكن هناك عملية مختلفة نشطة.
- **فلترة أقوى لمزامنة القروبات:** عناصر مثل Manage groups / New group / Archived / Communities / Search ونظائرها العربية لا تُحفظ كقروبات.
- **سياسة Pure Kotlin لأسماء المحادثات:** يمكن اختبار قواعد اكتشاف القروبات بدون Android أو WhatsApp فعلي.
- **حزمة Regression جديدة:** `scripts/run_stability_4_0_checks.sh` تفحص المصدر، التكامل، المحركات Pure Kotlin، قفل العمليات، فلترة القروبات، وملفات XML.
- **Legacy regression أصبح يوضح مرحلة Compile وRun** بدل ظهور Timeout غير مفهوم.
- ما زالت قيود WhatsApp والتقييد/Retry Later حدودًا إلزامية؛ لا توجد محاولة لتجاوزها.

## البناء والتحقق

المشروع يستهدف Android SDK 36 وJVM 17. GitHub Actions المرفق يجهز JDK/Android SDK/Gradle ويشغّل الاختبارات وLint ثم يبني Debug APK. إذا كان Gradle مثبتًا محليًا، يمكن إنشاء Wrapper الرسمي بواسطة `scripts/generate-gradle-wrapper.sh`.

---

## تاريخ الإصدارات — 2.8.0 Accessibility Visual Join & Fast Existing-Chat Handoff
