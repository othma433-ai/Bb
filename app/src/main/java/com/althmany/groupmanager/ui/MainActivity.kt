...
class MainActivity : AppCompatActivity() {
    ...
    private fun configureToolbar() {
        ...
        binding.toolbar.setOnMenuItemClickListener { item ->
            when (item.itemId) {
                ...
                R.id.action_extractor_workspace -> {
                    if (app.preferences.accessibilityBatchRunning) {
                        Toast.makeText(
                            this,
                            "أوقف تشغيل Sender الحالي أولاً قبل فتح أدوات الاستخراج والفحص والنشر",
                            Toast.LENGTH_LONG
                        ).show()
                    } else {
                        startActivity(Intent(this, com.althmany.extractor.MainActivity::class.java))
                    }
                    true
                }
                else -> false
    ...
    private fun startAutomaticRun(allowQueuedContinuation: Boolean = false) {
        ...
        if (allowQueuedContinuation && canContinueQueuedBatch() && queuedSnapshot != null) {
            ...
            if (!app.preferences.startAccessibilityBatch(queuedSnapshot.sessionId)) {
                Toast.makeText(this, "هناك عملية أخرى تتحكم بواجهة واتساب. أوقفها أولاً.", Toast.LENGTH_LONG).show()
                return
            }
        ...
        when (event) {
            ...
            is MainEvent.AutomaticSessionReady -> {
                ...
                if (requestedStartAt > System.currentTimeMillis() + 1_000L) {
                    ...
                    if (!lockValidatedRuntimeTarget()) return
                    if (!app.preferences.startAccessibilityBatch(event.sessionId)) {
                        Toast.makeText(this, "هناك عملية أخرى تتحكم بواجهة واتساب. أوقفها أولاً.", Toast.LENGTH_LONG).show()
                        return
                    }
