...
class QuickJoinAccessibilityService : AccessibilityService() {
    ...
    private fun accessibilityRuntimeActive(): Boolean {
        ...
        if (!RuntimeOperationCoordinator.ensureOwned(RuntimeOperation.SENDER)) {
            RuntimeDiagnosticStore.append(
                this,
                "UI_OPERATION_COLLISION",
                "Sender blocked because ${RuntimeOperationCoordinator.current()?.name ?: "UNKNOWN"} owns WhatsApp UI"
            )
            return false
        }
    ...
    private suspend fun launchScheduledBatch() {
        ...
        val launchState = withContext(Dispatchers.IO) {
            ...
            if (!app.preferences.startAccessibilityBatch(sessionId)) {
                RuntimeDiagnosticStore.append(
                    this@QuickJoinAccessibilityService,
                    "UI_OPERATION_COLLISION",
                    "Scheduled Sender start blocked by active ${RuntimeOperationCoordinator.current()?.name ?: "UNKNOWN"} operation"
                )
                return@withContext null
            }
