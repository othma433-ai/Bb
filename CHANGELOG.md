## 4.0.0 — Stability & Smart Automation Release — 2026-09-16

- Added a shared WhatsApp UI ownership gate covering Sender, Extraction, Scan, and Publish.
- Sender now reacquires its own lease idempotently after Accessibility/Shizuku service recreation, but never displaces another active engine.
- Extractor controller entry points now reject start/resume/sync when a persisted Sender batch is active, including non-UI entry paths.
- Added `ConversationTitlePolicy` to reject WhatsApp toolbar/filter/system actions from group synchronization while preserving legitimate names that merely contain words such as “group” or “archived”.
- Added focused pure regressions for ownership, Sender guards, and conversation-title filtering.
- Added `run_stability_4_0_checks.sh` and explicit legacy compile/run markers for deterministic diagnostics.
- Bumped Android app version to 4.0.0 / versionCode 400.
- Preserved restriction hard stops, profile/package locking, Shizuku/Accessibility fallback, extraction checkpoints, scan, publish, and export behavior.
